...
- 扩展固定评测 Runner，使 `/eval/run` 能执行真实 HITL checkpoint 恢复、无效令牌阻断和重复恢复幂等检查。
- 固定回归集补充已发货退款阻断、退款进度、商品 Tool + RAG、签收后退货、RRF 检索及 HITL 恢复场景。
- 保留 Prompt 按需检索和商业 Reranker 显式开启策略，避免退回全量 Prompt 或默认引入额外网络调用。
- FastAPI、能力声明、Trace 和测试断言版本同步升级到 `0.35.0`。
